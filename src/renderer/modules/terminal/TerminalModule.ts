...
export class TerminalModule {
  ...
  async createTerminal(): Promise<void> {
    ...
    const cmdInput =
      shell === 'cmd'
        ? new CmdLineInput(term, (payload) => window.electronAPI.writeTerminal(id, payload))
        : undefined;
    term.onData((data) => {
      if (cmdInput) {
        cmdInput.handleData(data);
        return;
      }
      // Discard mouse-report sequences generated by xterm when the shell
      // (e.g. PSReadLine) enables mouse tracking.  Without a real PTY the
      // sequences cannot be correlated with screen geometry, so they arrive
      // as garbage input and cause visible cursor jumps when the user clicks
      // anywhere in the terminal pane.
      //   X10 / normal button:  ESC [ M <btn> <col> <row>
      //   SGR extended:         ESC [ < ... M / m
